...
- **可读标签引用闭环:agent 引用标签、来源表解析标签、校验器识别标签。** 承接上一条改动:
  ...
  报告中的 `[...]` 标记按 `display_label`/标题/`S#` id 三种键解析回证据 id(双模,兼容历史
  报告里的 `[S1]`,同名标签会命中全部对应 id);来源表首列改为渲染 `display_label`(无标签时
  回退 `[S#]`);报告装配 (`build_markdown_report`) 与校验器 (`_citation_warnings`) 改用新解析器。
  副作用:校验器不再产生「无效引用」逐条告警(未知标记被静默忽略),保留「未发现来源引用」告警。
- **报告内联引用改用可读的数据/用途标签,不再显示 `[S1]` 等编号。** 工具输出呈现给 agent 与最终
  报告的证据前缀,现在渲染证据项的中文数据标签——`[历史行情（OHLCV）]`、`[已验证市场快照]`、
  `[10 日 EMA]`、`[MACD 柱状图（macdh）]`、`[资产负债表]`、`[综合基本面]` 等。标签在证据**注册期**
  按 `tool_name`/指标参数派生(与 run 内 `S#` 序号无关),存于证据项新字段 `display_label`;
  `prefix_with_evidence()` 从当前证据登记表解析该标签,无法解析时回退到原标题、再回退到 `[S#]`,
  确保出处不会被静默抹掉。`S#` 编号仍保留在登记表内部,用于去重、报告校验、来源表查询与持久化;
  来源表列、链接消毒与导出端点均未改动。
